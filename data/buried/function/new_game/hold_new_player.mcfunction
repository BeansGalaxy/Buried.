...
execute as @e[type=#buried:avatar] if score @s buried.new_game > :new_game.players_waiting buried run scoreboard players operation :new_game.players_waiting buried = @s buried.new_game
scoreboard players operation :new_game.players_waiting buried += 1 buried
scoreboard players operation @s buried.new_game = :new_game.players_waiting buried
...
scoreboard players operation @e[tag=new_game.temp] buried.new_game = @s buried.new_game
...
execute unless function buried:new_game/spread_players unless function buried:new_game/spread_players run function buried:new_game/spread_players

execute as @e[type=#buried:avatar,scores={buried.new_game=0..}] at @s run function buried:new_game/hold_player_after_spread
